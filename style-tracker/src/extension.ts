...
let trackingDisposables: vscode.Disposable[] = [];
...
async function openAndHighlightCSS(cssFilePath: string, ranges: vscode.Range[]) {
    ...
    try {
        ...
        let targetColumn: vscode.ViewColumn | undefined = undefined;
        ...
        if (cssEditor && decorationType) {
            try { cssEditor.setDecorations(decorationType, []); } catch { /* ignore */ }
        }
...
function clearHighlights() {
    if (cssEditor) {
            if (decorationType) cssEditor.setDecorations(decorationType, []);
    }
